...
Client.on("ready", () => {
	console.log("online");
	Client.user.setPresence({ game: { name: `Code créateur ; SUPERLEAGUEGAME`, type: 0} });
});
...
Client.on("message", async (message) => {
    ...
    if (message.content === prefix + `invite`){
        var help_embed = new Discord.RichEmbed()
            ...
            .addField("Invite", "``https://discordapp.com/oauth2/authorize?client_id=547454260988870686&scope=bot&permissions=8``")
    ...
    if (message.content === prefix + `soutien`){
        var help_embed = new Discord.RichEmbed()
            ...
            .addField("__Nombre de soutiens__", "``Acutalisation en cours ...``")
            .setFooter("Soutien nous : SUPERLEAGUEGAME")
        message.channel.sendEmbed(help_embed);
        //messagechannel
        console.log("Commande Help demandée !");
